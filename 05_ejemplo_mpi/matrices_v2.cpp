...
void matrices_v2(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        {
            std::vector<int> data = {MATRIX_DIH, rows_per_rank};
            MPI_Send(data.data(), 2, MPI_INT, i, 0, MPI_COMM_WORLD);

            int offset = i * rows_per_rank * MATRIX_DIH;
            MPI_Send(&A[offset], rows_per_rank * MATRIX_DIH, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);
        ...
        for (int i = 1; i < nprocs; i++)
        {
            int offset = i * rows_per_rank;
            MPI_Recv(X.data() + offset, rows_per_rank, MPI_DOUBLE, i, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
